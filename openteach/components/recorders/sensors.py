...
class ReskinSensorZMQRecorder(Recorder):
    def __init__(
        self,
        controller_configs,
        storage_path
    ):
        self._controller = hydra.utils.instantiate(controller_configs)
        self.timer = FrequencyTimer(RESKIN_FPS)

        self._filename = 'reskin_sensor_values'
        self.notify_component_start('{}'.format(self._filename))
        self._recorder_file_name = os.path.join(storage_path, self._filename + '.h5')
        self._host = controller_configs["stream_configs"]["host"]
        self._sensor_stream_port = controller_configs["stream_configs"]["port"]
        
        self.sensor_information = defaultdict(list)

    def stream(self):
        print('Checking if Reskin sensors are active...')
        while self._controller.get_sensor_state() is None:
            continue
        print('Starting to record Reskin frames from port: {}'.format(self._sensor_stream_port))
        ...
        with h5py.File(self._recorder_file_name, 'w') as hf:
            for key in self.sensor_information.keys():
                if key != 'timestamp':
                    self.sensor_information[key] = np.array(self.sensor_information[key], dtype=np.float32)
                    hf.create_dataset(key, data = self.sensor_information[key], compression="gzip", compression_opts = 6)
                else:
                    self.sensor_information['timestamp'] = np.array(self.sensor_information['timestamp'], dtype = np.float64)
                    hf.create_dataset('timestamps', data = self.sensor_information['timestamp'], compression="gzip", compression_opts = 6)
